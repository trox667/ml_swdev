...
def back(X, Y, y_hat, w2, h):
    ...
    w1_gradient = np.matmul(prepend_bias(X).T, np.matmul(
        y_hat - Y, w2[1:].T) * sigmoid_gradient(h)) / X.shape[0]
...
def report(epoch, batch, X_train, Y_train, X_test, Y_test, w1, w2):
    ...
    print(f'{epoch}-{batch} > Loss: {training_loss:.8f}, Accuracy: {accuracy:.2f}')
...
if __name__ == '__main__':
    import mnist
    w1, w2 = train(mnist.X_train, mnist.Y_train, mnist.X_test,
                   mnist.Y_test, 200, 2, 20000, 0.01)
